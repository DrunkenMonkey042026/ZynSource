...
applicationsRouter.post('/', requireAuth, requireRole('seeker'), async (req, res, next) => {
  try {
    ...
    if (!profile?.resumeUrl) throw new HttpError(400, 'Please upload a resume on your profile before applying')
...
applicationsRouter.get('/by-job/:jobId', requireAuth, requireRole('recruiter'), async (req, res, next) => {
  try {
    ...
    const seekers = await User.find({ _id: { $in: seekerIds } }).select('name email').lean()
